...
const Navbar = () => {
  ...
  return (
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed op-0 z-20 bg-primary `}>
      <div className='w-full flex justify-between items-center max-w-7x1 mx-auto'>
        ...
        window.scroll(0,0)
          ...
          <p className='text-white cursor-pointer text-[18px] font-bold flex'> Saiteja &nbsp;
       ...
       <div className='sm:hidden flex flex-1 justify-end items-center'>
        ...
        <ul className='list-none flex sm:flex justfiy-end items-start flex-col gap-4'>
          {
            navLinks.map((Link)=>(
              <li key={Link.id} className={`${active=== Link.title ?"text-white": "text-secondary"} 
